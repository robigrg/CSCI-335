// Robi Gurung
// avl_tree_mod.h : modified version of avl_tree.h
...
class AvlTree
{
  ...
  private:
    ...
    // helper to count and returns the total number of nodese
    int totalNodesHelper( AvlNode *t )const 
	{
        int count = 0;
        if( t != nullptr ) {
    ...
    int depthHelper(AvlNode *t, int depth) 
	{
        if (t == nullptr)
            return 0;
        else
    ...
    // helper to print the element in x
